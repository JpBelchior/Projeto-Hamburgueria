...
export const create = async (req: Request, res: Response): Promise<void> => {
  try {
    const { name, cpf, email, password, roles, cargo, salario, dataAdmissao } = req.body;

    if (!name || !cpf || !email || !password || !roles || !cargo || !salario) {
      res.status(400).json({
        message: "Campos obrigatórios: name, cpf, email, password, roles, cargo, salario.",
      });
      return;
    }

    const funcionario = await FuncionarioService.create({
      name, cpf, email, password, roles, cargo, salario: Number(salario), dataAdmissao,
    });
